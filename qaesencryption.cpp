 ...
 * Static Functions
 * */
QByteArray QAESEncryption::DefCrypt(const QByteArray &rawText, const QByteArray &key,
                                    const QByteArray &iv) {
    return Crypt(AES_256, CFB, rawText, key, iv, Padding::ZERO);
}

QByteArray QAESEncryption::DefDecrypt(const QByteArray &rawText, const QByteArray &key,
                                      const QByteArray &iv) {
    return Decrypt(AES_256, CFB, rawText, key, iv, Padding::ZERO);
}

QByteArray QAESEncryption::Crypt(QAESEncryption::Aes level, QAESEncryption::Mode mode,
                                 const QByteArray &rawText, const QByteArray &key,
                                 const QByteArray &iv, QAESEncryption::Padding padding) {
    return QAESEncryption(level, mode, padding).encode(rawText, key, iv);
}

QByteArray QAESEncryption::Decrypt(QAESEncryption::Aes level, QAESEncryption::Mode mode,
                                   const QByteArray &rawText, const QByteArray &key,
                                   const QByteArray &iv, QAESEncryption::Padding padding) {
    return QAESEncryption(level, mode, padding).decode(rawText, key, iv);
}

QByteArray QAESEncryption::ExpandKey(QAESEncryption::Aes level, QAESEncryption::Mode mode,
...
QByteArray QAESEncryption::expandKey(const QByteArray &key) {
    ...
    for (i = m_nk; i < m_nb * (m_nr + 1); i++) {
        ...
        if (i % m_nk == 0) {
            ...
            // qDebug() << i / m_nk << QString("%1").arg(Rcon[i / m_nk], 2, 16);
        }
...
void QAESEncryption::invMixColumns() {
    ...
    for (int i = 0; i < 16; i += 4) {
        ...
        it[i] =
            (quint8)(multiply(a, 0x0e) ^ multiply(b, 0x0b) ^ multiply(c, 0x0d) ^ multiply(d, 0x09));
        it[i + 1] =
            (quint8)(multiply(a, 0x09) ^ multiply(b, 0x0e) ^ multiply(c, 0x0b) ^ multiply(d, 0x0d));
        it[i + 2] =
            (quint8)(multiply(a, 0x0d) ^ multiply(b, 0x09) ^ multiply(c, 0x0e) ^ multiply(d, 0x0b));
        it[i + 3] =
            (quint8)(multiply(a, 0x0b) ^ multiply(b, 0x0d) ^ multiply(c, 0x09) ^ multiply(d, 0x0e));
    }
}
...
QByteArray QAESEncryption::encode(const QByteArray &rawText, const QByteArray &key,
                                  const QByteArray &iv) {
...
QByteArray QAESEncryption::decode(const QByteArray &rawText, const QByteArray &key,
                                  const QByteArray &iv) {
    if (m_mode >= CBC && (iv.isNull() || iv.size() != m_blocklen))
        return QByteArray();
